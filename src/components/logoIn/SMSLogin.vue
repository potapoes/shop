    ...
    <div class="logoin-main" ref="con">
        ...
        <div class="from">
            <div class="input-main phone-number">
                ...
                <input type="text" oninput="if(value.length > 11)value=value.slice(0, 11)" placeholder="请输入验证手机号码" @blur="hid" @focus="hidden" v-model="mobile">
            </div>
        ...
        methods:{
            ...
            obtain : function(){
                ...
                var N = 60,
                    _this = this,
                    clear = null;
                this.axios.post(this.$httpConfig.sms,qs.stringify({
                    mobile:this.mobile
                })).then((res) => {
            ...
            logoIn:function(){//帐户名密码是否正确如果正确跳转主页
                ...
                this.axios.post(this.$httpConfig.smsLogin,qs.stringify({
                    mobile:this.mobile,
                    verify:this.message,
                    token: this.token
                })).then((res) => {
                    ...
                    if(res.data.status == 1){
                        localStorage.setItem('user_ID', res.data.data.token);
