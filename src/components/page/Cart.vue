...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            subtract(n,_id){
                this.load = true;
                this.axios.post('/api/Home/Cart/addReduce',qs.stringify({
            ...
            remove(index,item){//从购物车中删除商品
                MessageBox.confirm('确定删除此商品?').then(action => {
                    this.axios.post('/api/Home/Cart/delete',qs.stringify({
                        app_user_id:sessionStorage.getItem('user_ID'),
                        id:item.id
                    })).then((res) => {
            ...
            collection(){//添加到收藏
                ...
                this.axios.post('/api/Home/Cart/muchCollection',qs.stringify({
                    app_user_id:sessionStorage.getItem('user_ID'),
